...
namespace MapleLib.PacketLib {
	...
	public class Session {
					...
					switch (socketInfo.State) {
							...
							if (socketInfo.NoEncryption) {
								...
								if (_type == SessionType.SERVER_TO_CLIENT &&
									...
									Helpers.ErrorLogger.Log(Helpers.ErrorLevel.Critical,
										"[Error] Packet check failed. Disconnecting client.");
								//this.Socket.Close();
								socketInfo.State = SocketInfo.StateEnum.Content;
						...
						case SocketInfo.StateEnum.Content:
							...
							if (socketInfo.NoEncryption) {
								...
								if (_type == SessionType.CLIENT_TO_SERVER) OnInitPacketReceived(version, serverType);
		...
		public void SendPacket(byte[] input) {
			...
			var header = _type == SessionType.SERVER_TO_CLIENT
